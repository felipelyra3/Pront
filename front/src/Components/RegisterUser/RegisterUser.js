...
import { registerNewUserApi } from "../../Api/RegisterApi";
import { useNavigate } from "react-router-dom";
import { OutterFieldset } from "../../Styles/GlobalStyles";
...
export default function RegisterUser() {
    ...
    function handleForm(e) {
        ...
        body.password = (cpf.slice(0, 4)) + (socialName.slice(1, 3)) + (year);
        const config = {
            headers: {
        ...
        const post = registerNewUserApi(body, config);
        ...
        post.catch((error) => {
            ...
            if (error.response.status === 409) {
                alert("CPF ou número do SUS já cadastrados");
                return
            }
        ...
        <DefaultLoggedPage>
            <OutterFieldset>
                ...
                <Form onSubmit={handleForm}>
                    <LabelWrapper>
                        <div className="name">Nome completo: <input type="text" id="name" placeholder="Nome completo" value={name} size="100%" onChange={(e) => { setName(e.target.value) }} required></input></div>
                        <div className="socialName">Nome social: <input type="text" id="socialName" placeholder="Nome social" value={socialName} onChange={(e) => { setSocialName(e.target.value) }}></input></div>
                        ...
                        <div className="CPF-Email-SUS">
                            ...
                            <div className="susNumber">Número do SUS: <input type="number" id="susNumber" placeholder="Número do SUS" value={susNumber} onChange={(e) => { setSusNumber(e.target.value) }}></input></div>
                        </div>
                        ...
                        {session.loginType === "admin" ?
                            ...
                            <></>}
                        <ButtonWrapper><Button>Cadastrar</Button></ButtonWrapper>
                    </LabelWrapper>
                </Form>
            </OutterFieldset>
        </DefaultLoggedPage>
